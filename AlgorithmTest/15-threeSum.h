...
std::vector<std::vector<int>> CThreeSum::threeSum_0(vector<int>& nums)
{
	vector<vector<int>>vecThreeSum;
	int nSize = nums.size();
	if (nSize < 3)
		...
		while (nfirst < nSize -2)
		{
			...
			while (nCurrent < nLast)
			{
				...
				if (nums[nfirst] + nums[nCurrent] + nums[nLast] == 0)
				{
					...
					while (nums[nLast--] == nums[nLast] && nLast > nCurrent);
				}
				else
				{
					if (nums[nfirst] + nums[nCurrent] + nums[nLast] < 0)
...
void CThreeSum::TestThreeSum_0()
{
	int arrValue[] = { /*0,0,0,0,0*//*-1,0,1,2,-1,-4*/ /*1,-1,-1,0*/ -2,0,0,2,2 };
